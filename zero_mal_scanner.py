...
class ScanHistory:
    ...
    def add(self, scan_result: Dict):
        ...
        if self.persist_to_file and len(self.history) % 100 == 0:
            self.save_to_file()
...
class ZeroMALGUI:
    ...
    def setup_ui(self):
        ...
        features = ["⚡ 4x Parallel Scan", "🔍 Magic Number Detection", "💾 Auto History Save", "📊 Real-time Stats"]
        ...
        info_frame2 = tk.Frame(right_edge, bg='#1a1a1a', relief=tk.RAISED, bd=1)
        info_frame2.pack(fill=tk.X, pady=10)
        
        tk.Label(info_frame2, text="ℹ️ INFO", font=('Arial', 10, 'bold'),
                fg='#00ff00', bg='#1a1a1a').pack(pady=5)
        tk.Label(info_frame2, text="Auto-saves every 100 scans", 
                fg='#888888', bg='#1a1a1a', font=('Arial', 8)).pack()
        tk.Label(info_frame2, text="Max history: 1000 entries", 
                fg='#888888', bg='#1a1a1a', font=('Arial', 8)).pack()
        tk.Label(info_frame2, text="Magic number detection active", 
    ...
    def run_parallel_scan(self):
        ...
        try:
            ...
            for result in results:
                if result.get('status') != 'STOPPED':
                    self.root.after(0, lambda r=result: self._process_result_safe(r))
                    
                    if result['status'] == 'MALWARE_DETECTED':
                        self.total_malware += 1
                        self.root.after(0, lambda: self.threats_var.set(str(self.total_malware)))
            
            self.root.after(0, lambda: self.history_var.set(str(self.scanner.history.get_size())))
            
            if len(self.scanner.history.get_all()) > 0:
                self.root.after(0, lambda: self.export_btn.config(state='normal'))
                
        except Exception as e:
            self.root.after(0, lambda: self.log_message(f"Scan error: {str(e)}"))
        finally:
            self.root.after(0, self.scan_complete)
    
    def _process_result_safe(self, result):
        try:
            if result['status'] == 'MALWARE_DETECTED':
                for threat in result['threats']:
                    ...
                    self.log_message(f"⚠️ {threat['severity']}: {threat['type']} - {result['file_name']}")
            else:
                file_type = result.get('file_type', {}).get('description', 'Unknown')
                self.tree.insert('', 'end', values=(
                    ...
                    result['file_name'],
                    file_type,
                    result['verification'][0] if result['verification'] else 'Verified clean',
                    'N/A'
                ), tags=('CLEAN',))
        except Exception as e:
            self.log_message(f"Error processing result: {e}")
    
    def export_history(self):
        if self.scanner.history.get_size() == 0:
            messagebox.showwarning("No History", "No scan history to export")
            return
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"zero_mal_history_{timestamp}.json"
        
        success, result = self.scanner.export_history(filename)
        if success:
            messagebox.showinfo("Export Complete", f"History exported to: {result}")
            self.log_message(f"History exported: {result}")
        else:
            messagebox.showerror("Export Failed", f"Error: {result}")
    
    def log_message(self, message):
        timestamp = datetime.now().strftime("%H:%M:%S")
        self.root.after(0, lambda: self._log_safe(f"[{timestamp}] {message}\n"))
    ...
    def scan_complete(self):
        ...
        if self.stop_scan_flag:
            self.status_label.config(text=f"Stopped - Scanned {self.total_scanned} files")
            self.log_message(f"Scan stopped. Scanned {self.total_scanned} files")
        else:
            self.status_label.config(text=f"Complete - Scanned {self.total_scanned} files")
            self.log_message(f"Scan completed. Scanned {self.total_scanned} files")
            self.log_message(f"History saved: {self.scanner.history.get_size()} entries")
        
        stats = self.scanner.get_history_stats()
        self.log_message(f"Statistics: {stats['malware_count']} threats / {stats['clean_count']} clean files")
        
        if self.total_malware > 0:
            self.root.after(0, lambda: messagebox.showwarning("Scan Complete", 
                f"Found {self.total_malware} confirmed threats!\n\n"
                f"Files scanned: {self.total_scanned}\n"
                f"Malware detected: {self.total_malware}\n\n"
                f"History saved. Use Export History to save results."))
        else:
            if not self.stop_scan_flag:
                self.root.after(0, lambda: messagebox.showinfo("Scan Complete", 
                    f"No threats detected!\n\n"
                    f"Files scanned: {self.total_scanned}\n"
                    f"All files are clean.\n\n"
                    f"History saved for this session."))
...
def main():
    ...
    print("""
    ╔══════════════════════════════════════════════════════════════════╗
    ║                 Zero MAL Scanner v1.0                            ║
    ║                 Malware Detection System                         ║
    ║                 Parallel Scan | Magic Numbers | Auto-History     ║
    ╚══════════════════════════════════════════════════════════════════╝
    """)
